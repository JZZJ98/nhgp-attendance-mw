export default async function handler(req, res) {
  try {
    ...
    // If memory missing (cross-instance), build minimal pass from signed cookies
    if (!pass) {
      ...
      if (meta && sig) {
        ...
        if (parts.length === 3 && parts[0] === jti) {
          pass = { site_id: parts[1], expAt: Number(parts[2] || 0), redeemed:false, used:false };
          // (We re-verify signature later in webhook; this is just to proceed with redirect)
        }
      }
    }
    ...
    formUrl.searchParams.set('exp', String(expAt));
    // (No "Access Gate" visual field anymore — relying on server-side webhook enforcement)
